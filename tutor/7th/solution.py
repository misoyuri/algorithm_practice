...
class BinarySearchTree:
    ...
    def insert(self, root: Node, val: T) -> None:
        ...
        def inner_insert(base_node, val) -> int:
            if base_node.value > val:
                if base_node.left != None:
                    inner_insert(base_node.left, val)
                else:
                    base_node.left = Node(val, parent=base_node)
                    self.size += 1
            elif base_node.value < val:
               if base_node.right != None:
                    inner_insert(base_node.right, val)
               else:
                   base_node.right = Node(val, parent=base_node)
                   self.size += 1
                   
            base_node.height = self.height(base_node)
            
        inner_insert(base_node=root, val=val)
        
        
    def remove(self, root: Node, val: T) -> Optional[Node]:
        def inner_remove(node, val):
            if node is None:
                return None, None
            
            elif val < node.value:
                node.left, rem_node = inner_remove(node.left, val)
                node.height = self.height(node)
                
            elif val > node.value:
                node.right, rem_node = inner_remove(node.right, val)
                node.height = self.height(node)
                ...
                if node.left is None and node.right is None:
                    ...
                    node.left, rem_node = inner_remove(node.left, replace.value)

            return node, rem_node
        
        root, removed_node = inner_remove(root, val)
    ...
    def search(self, root: Node, val: T) -> Optional[Node]:
        ...
        while base_node:
            if base_node.value == val:
                ...
                if not base_node.left:
                    break
                base_node = base_node.left
            else:
                if not base_node.right:
                    break
